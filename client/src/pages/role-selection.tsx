        ...
        {roles.map((role, i) => (
          <Link key={role.id} href={`/chat?role=${role.id}`} className="block h-full">
            <motion.div
              ...
              className="group relative flex flex-col p-8 rounded-xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.05] hover:border-primary/50 transition-all cursor-none interactive h-full"
            >
              <div className="absolute top-4 right-4 font-mono text-[10px] text-muted-foreground/50 border border-white/5 px-2 py-1 rounded">
                {role.accessLevel}
              </div>
