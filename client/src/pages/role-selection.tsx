...
const roles = [
  ...
  {
    id: "govt",
    title: "Developer / Government Agent",
    icon: Landmark,
...
export default function RoleSelectionPage() {
  ...
  const handleRoleClick = (roleId: string) => {
    if (roleId === "citizen" || roleId === "business") {
      navigate(`/chat?role=${roleId}`);
      return;
    }

    if (roleId === "drafter") {
      navigate("/draft/select"); // 👈 NEW INTERMEDIATE PAGE
      return;
    }
    ...
    <div className="min-h-[80vh] flex flex-col items-center justify-center container mx-auto px-6 py-12">
      ...
      >
        ...
        <p className="text-muted-foreground max-w-xl mx-auto">
          The SwaRakshak Legal Reasoning Engine adapts its output based on your selected role.
        </p>
      </motion.div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
        {roles.map((role, i) => {
          const roleImages = {
            citizen: "https://images.unsplash.com/photo-1454923634634-bd1614719a7b?q=80&w=800&auto=format&fit=crop",
          ...
          return (
            <motion.div
              ...
              className="group relative flex flex-col p-8 rounded-xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.05] hover:border-primary/50 transition-all cursor-pointer h-full overflow-hidden"
            >
              <img
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center text-xs font-bold uppercase tracking-wider text-primary opacity-0 group-hover:opacity-100 transition-all">
                  Initialize →
                </div>
              </div>
            </motion.div>
